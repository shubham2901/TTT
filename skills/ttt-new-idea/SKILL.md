---
name: ttt-new-idea
description: Start the TTT flow — vague idea to validated, scoped, build-ready spec (research, plan, handoff).
---
...
Start the TTT flow. Takes a vague idea and produces a validated, scoped, build-ready spec.

## Usage

`/ttt-new-idea` or say **TTT** / **To The T** / **product spec** in chat (when the skill is installed).

## What It Does

1. **Clarify** — Narrows your idea until it is specific enough to research (max 2 questions per turn).
2. **Validate** — Runs combined market + user research using the **researcher** subagent → `research.md`.
3. **Define** — Locks scope (max 5 features for v1), primary user, one core problem, realistic first milestone.
4. **Specify** — Writes `plan.md`, then `handoff.md` for your coding tool; optional `screens.md` / `tests.md`.
5. **Plan lock** — On **"lock it"**, outputs a plain-English chat summary; `summary.md` only if the user says **"save summary"**.

## Output

Under `{artifact_root}` (default `ttt/`; fallbacks `ttt-docs/`, `ttt-artifacts/` if `ttt/` is taken — persisted in state):

- `ttt_state.json` — session progress and decisions
- `research.md`, `plan.md`, `handoff.md` — core deliverables
- Optional: `screens.md`, `tests.md`, `summary.md`

## Integration

- **Research subagent:** Use `ttt/agents/researcher.md` after install, or `prompts/researcher.md` from the TTT repo.
- Follow the **Prompt** below as the choreographer system instruction.
